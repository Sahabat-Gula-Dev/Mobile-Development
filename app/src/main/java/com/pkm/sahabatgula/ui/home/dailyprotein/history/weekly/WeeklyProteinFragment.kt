...
@AndroidEntryPoint
class WeeklyProteinFragment : Fragment() {

    private var _binding: FragmentWeeklyHistoryBinding? = null
    private val binding get() = _binding!!
    private val viewModel: WeeklyProteinViewModel by viewModels()
    ...
    private fun observeUiState() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.uiState.collect { state ->
                when (state) {
                    is WeeklyProteinState.Loading -> {
                    }
                    is WeeklyProteinState.Success -> {
                        setupBarChart(binding.weeklyChart, state.barData, state.xAxisLabels)
                    }
                    is WeeklyProteinState.Error -> {
                    }
                }
            }
        }
