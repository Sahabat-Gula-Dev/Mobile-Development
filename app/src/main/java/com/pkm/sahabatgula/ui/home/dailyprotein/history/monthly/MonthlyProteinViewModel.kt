package com.pkm.sahabatgula.ui.home.dailyprotein.history.monthly
...
sealed class MonthlyProteinState {
    object Loading : MonthlyProteinState()
    data class Success(
        val barData: BarData,
        val xAxisLabels: List<String>
    ) : MonthlyProteinState()
    data class Error(val message: String) : MonthlyProteinState()
}


@HiltViewModel
class MonthlyProteinViewModel @Inject constructor(
    private val homeRepository: HomeRepository
) : ViewModel() {

    private val _uiState = MutableStateFlow<MonthlyProteinState>(MonthlyProteinState.Loading)
    val uiState: StateFlow<MonthlyProteinState> = _uiState

    init {
        loadMonthlyProteinData()
    }

    private fun loadMonthlyProteinData() {
        viewModelScope.launch {
            _uiState.value = MonthlyProteinState.Loading
            val monthlyData = homeRepository.observeMonthlySummary().firstOrNull()
            if (monthlyData.isNullOrEmpty()) {
                _uiState.value = MonthlyProteinState.Error("Data bulanan tidak ditemukan.")
                return@launch
            }
    ...
    private fun processDataForChart(monthlyData: List<SummaryEntity>) {
        ...
        monthSlots.forEachIndexed { index, yearMonth ->
            ...
            val proteinAmount = dataForMonth?.carbs ?: 0.0
            entries.add(BarEntry(index.toFloat(), proteinAmount.toFloat()))
        ...
        _uiState.value = MonthlyProteinState.Success(barData, xAxisLabels)
    }
}